...
from long_document_indexing.cli import (
    _evaluate_foundry_managed,
    _foundry_managed_result_path,
    _load_foundry_managed_result,
)
...
def test_report_loaders_ignore_stale_foundry_managed_result(tmp_path: Path) -> None:
    config = load_experiment_config(
        Path("configs/experiments/foundry-multilexsum-legal-rag-qa-smoke.yaml"),
        project_root=Path.cwd(),
    )
    ...
    store.write_json(config.evaluation.foundry.manifest_path, {"dataset_sha256": "current"})
    store.write_json(
        config.evaluation.foundry.result_path,
        {"metadata": {"dataset_sha256": "stale"}},
    )

    assert _load_foundry_managed_result(config, store) is None
    assert _foundry_managed_result_path(config, store) is None
    ...
    assert _foundry_managed_result_path(config, store) is not None
